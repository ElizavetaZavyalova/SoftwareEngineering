...
driver_repository = DriverRepository(url=os.getenv("DATABASE_DRIVER_URL", "postgresql://root:root@localhost:5501/drivers"))
...
##########################################################
@app.post("/driver/trip/create", tags=[AddingTripsController._EDITING_TRIPS], summary="Создание поездки с текущим водителем")
...
@app.get("/driver/trips/", tags=[AddingTripsController._EDITING_TRIPS], summary="Получение всех поездок текущего водителя")
def get_all_trips(token: str = Depends(oauth2_scheme)):
    return routes.get_all_trips(token=token)


@app.get("/driver/trip/{id}/", tags=[AddingTripsController._EDITING_TRIPS], summary="Получение конкретной поездки по id")
def get_trip(id: int, token: str = Depends(oauth2_scheme)):
    return routes.get_trip(id=id, token=token)


@app.patch("/driver/trip/{id}/info", tags=[AddingTripsController._EDITING_TRIPS], summary="Обновление информации о поездке по id")
